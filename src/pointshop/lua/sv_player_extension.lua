...
function Player:PS_UpdateItems()
	if not self.PS_Items then
		self.PS_Items = POINTSHOP.ValidateItems(util.JSONToTable(self:GetPData("PointShop_Items", "")) or false) or {}
	end
	
	self:SetPData("PointShop_Items", util.TableToJSON(self.PS_Items))

	timer.Simple(1, function() 
		net.Start( "PointShop_Items" )
			net.WriteTable( self.PS_Items or {} )
		net.Send( self )
	end)
